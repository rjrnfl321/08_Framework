...
//		@RequestMapping(value = "test1", method = RequestMethod.GET)
	...
	public String test1(HttpServletRequest req) {
		...
		// 왜? Spring의 ArgumentResolver 때문에 가능하다
		...
		String inputAge = req.getParameter("inputName");
		...
		// classpath:/templates/param/result.html로 forward
		...
		// - 위임 시 HttpServletRequset, HttpServletResponse를
		// 같이 위임
	}
	 ...
	 * 
	 * - request객체를 이용한 파라미터 전달 어노테이션 - 매개 변수 앞에 해당 어노테이션을 작성하면, 매개변수에 값이 주입됨. -
	 ...
	 * 존재하지 않는다면 400 Bad Request 에러 발생 -> required = true인 파라미터가 null인 경우에도 400 Bad
	 * Request
	 * 
	...
	@PostMapping("test2")
	public String test2(@RequestParam("title") String t, @RequestParam("writer") String w, @RequestParam("price") int p,
		...
		/*
		 * log를 이용해서 파라미터 출력하기 1) 클래스 위에 @Slf4j 어노테이션 추가 (Lombok 제공) 2) log.debug() 또는
		 * log.info() 등을 이용해서 log 출력
		 */

		log.debug("title : {} ", t); // {} 안에 t값이 대입되어 출력됨
		log.debug("writer : {} ", w);
		log.debug("price : {} ", p);
	...
	/*
	 * 3. @RequestParam 이용하기 - 여러 개 (복수, 다수) 파라미터 컨트롤러에서 얻는 방법
	 */
	// 1) String[]
	// 2) List<자료형>
	// 3) Map<String, Object>

	@PostMapping("test3") // /param/test3 POST 방식 요청을 처리하는 메서드
	...
	public String test3(
			...
			// 파라미터 중 "fruit"만 모아서 List<> 로 전달 받기
			// 단, 필수는 아니다
		...
		// templates 폴더 -> param 폴더 -> result3에 forward 하겠다
		// forward : 요청 위임
	}
